...
if __name__ == '__main__':
    ...
    parser.add_argument('--dropedge_percent', type=float, default=0.9)
    parser.add_argument('--aug', type=strtobool, default=True)

    parser.add_argument('--cls-balance', type=strtobool, default=True, help='whether to balance the cls when training and testing')
    parser.add_argument('--repeats', type=int, default=2, help='how many times to repeat the experiments for the mean and std')
    ...
    for hyp_params in hyp_param_list:
        ...
        name = f'{subfolder}val_{args.dataset}_{args.n_cls_per_task}_{args.method}_{list(hyp_params.values())}_{args.backbone}_{backbone_args[args.backbone]}_{args.classifier_increase}_{args.cls_balance}_{args.epochs}_{args.repeats}_Aug:{args.aug}'
        if args.minibatch:
