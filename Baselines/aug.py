...
def aug_random(graph):
    randval = np.random.randint(3)
    if randval == 0:
        # aug_graph = aug_drop_node(graph, drop_percent=0.1)
        # print("drop node success!")
        aug_graph = graph
    elif randval == 1:
        aug_graph = aug_drop_add_link(graph, drop_percent=0.2)
        # print("edge modification success!")
    elif randval == 2:
        aug_graph = aug_mask(graph, drop_percent=0.2)
        # print("masking success!")
    # elif randval == 3:
    #     aug_graph = graph
        # aug_graph = aug_subgraph(graph, drop_percent=0.4)
...
def aug_drop_add_link(graph, drop_percent=0.2):
    ...
    for i in range(drop_num):
        ...
        if del_edge_id1.size(0) and (del_edge_id1.item() in all_edges_id_list):
            del_edges_id_list.append(del_edge_id1)
            all_edges_id_list.remove(del_edge_id1.item())
        if del_edge_id2.size(0) and (del_edge_id2.item() in all_edges_id_list):
    ...
    d = torch.tensor(random.sample(l, add_num)).cuda()
